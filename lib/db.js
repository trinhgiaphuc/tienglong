...
export const addHeart = async (wordId, authorId) => {
  ...
  // BEST SOLUTION
  // heartCol => userDoc => {
  //   => wordIdCol => likerIdDoc
  //   => heartCount: author's heart count
  //   => authorIdCol => wordIdDoc => heartField: heart's counter
  const authorWordHeartRef = doc(db, 'users', authorId, 'hearts', wordId);

  // heartsCol => WordIdDoc => {
  //   => heartsCountField: number of author's hearts
  //   => wordIdField: number of word's hearts
  //   => heartsCol => wordIdDoc
  //   => heartCountField: word's heart counter
  // }
