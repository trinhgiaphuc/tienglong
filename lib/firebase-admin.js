...
export async function verifyFirebaseToken(token) {
  try {
    const result = await auth.verifySessionCookie(token);
...
export async function countUserWordsAndHearts(uid) {
  try {
    const doc = await firestoreAdmin.collection('user-words').doc(uid).get();
    if (!doc.exists) {
...
function getReactionRefs(wordId, authorId, likerId) {
  ...
  const wordHeartRef = firestoreAdmin.doc(`user-words/${authorId}`);
  const wordRef = firestoreAdmin.doc(`user-words/${authorId}/words/${wordId}`);
  return { heartRef, wordHeartRef, wordRef };
}

// Use last updated to check if request has been abused
export async function addHeart(wordId, authorId, likerId) {
  ...
  batch.update(refs.wordHeartRef, { hearts: firestore.FieldValue.increment(1) });
...
export async function reportWord(wordId, reporterId, message) {
  //TODO: implement check if report already existed
  const reportedWordRef = firestoreAdmin.collection('reports').doc(`${wordId}-${reporterId}`)
  try {
    await reportedWordRef.set({ message });
  } catch {
    console.error(error);
  }
}
