...
const WordCard = ({ word }) => {
  ...
  return (
    <article className="px-8 py-4 h-full tracking-wide font-mono flex flex-col gap-5 justify-around">
      <div className="flex flex-wrap gap-2 justify-center sm:justify-start sm:p-2">
        <Link href="/" passHref>
          <a className="rounded-3xl smaller-text-responsive p-4 bg-orange-400 text-center">
        ...
        {tags.map(tag => (
          <Link key={tag} href="/" passHref>
            <a className="rounded-3xl smaller-text-responsive p-4 bg-blue-400 text-center">
              {tag}
            </a>
          </Link>
        ))}
      ...
      <Link href={`/word/${id}`} passHref>
        <a className="cursor-pointer">
          ...
          <p className="text-black p-4 lg:p-2 text-responsive line-clamp-3 text-ellipsis">
            {definition}
          </p>
        </a>
      </Link>
